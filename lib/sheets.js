...
export async function setupSheets() {
  ...
  try {
    ...
    if (!sowCheck.data.values) {
      const sowHeaders = [["ID","Client Name","Service Type","Keywords/Details","Backlinks/Mo","SEO Tasks","Creatives Required","Priority","Status","Start Date","End Date","Notes"]];
      const sowSeed = [
        ["1","SummarizeX","SEO + Blog Management + PPC","12","90-100","47","-","A","Active","","",""],
        ["2","KN Packaging","Full SEO Package","15-20","90-100","46","-","B","Active","","",""],
        ["3","Beele","Full SEO Package + BLOGS","Full","90-100","46","-","B","Active","","",""],
        ["4","VOS","Full SEO Package","Full","90-100","46","-","B","Active","","",""],
        ["5","Pyarababy","Full SEO + PPC + Organic + BLOGS","Full","90-100","46","17","B","Active","","",""],
        ["6","Courtesy Honda","GMB Management (6 Locations) + Organic + PPC","GMB","70-80","14","25","C","Active","","",""],
        ["7","Shecare","SEO + Website + Blog + Organic + PPC","15","50-60","45","5","D","Inactive","","",""],
        ["8","SSW","PPC + Organic","-","-","-","25","A","Active","","",""],
        ["9","VOLVO","PPC + Organic","-","-","-","5","A","Active","","",""],
        ["10","VERISEEK","PPC + Organic","-","-","-","20","A","Active","","",""],
        ["11","ASIA","PPC + Organic","-","-","-","15","A","Active","","",""],
        ["12","LMK","Organic","-","-","-","15","B","Active","","",""],
        ["13","HONDA","PPC + Organic","-","-","-","25","C","Active","","",""],
        ["14","FAITH","PPC + Organic","-","-","-","10","D","Active","","",""],
        ["15","PRATHA","PPC + Organic","-","-","-","17","B","Active","","",""],
        ["16","Tress Lounge","Organic","-","-","-","5","B","Inactive","","",""],
        ["17","North International","PPC","-","-","-","4","C","Active","","",""],
        ["18","MANTHAN","PPC + Organic","-","-","-","12","B","Active","","",""],
        ["19","Softradix","PPC + Organic + 4 BLOGS","-","-","-","10","B","Active","","",""],
        ["20","OUTLANDER","PPC","-","-","-","15","B","Active","","",""],
        ["21","KIA","Organic","-","-","-","4","B","Active","","",""],
        ["22","Social Magnet","Organic","-","-","-","5","B","Active","","",""],
        ["23","BODY TEMPLE","PPC + GMB + Organic","-","-","-","15","B","Active","","",""],
      ];
      await sheets.spreadsheets.values.update({
        spreadsheetId: SPREADSHEET_ID,
...
export async function saveSOWRow(row) {
  ...
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
    range: `${SOW_SHEET_NAME}!A2:A1000`,
  });
  const ids = (res.data.values || []).map(r => String(r[0]));
  const rowValues = [
    row.id, row.clientName, row.serviceType, row.keywords,
    row.backlinks, row.seoTasks, row.creativesRequired,
    row.priority, row.status, row.startDate, row.endDate, row.notes || ""
  ];
  const idx = ids.findIndex(id => id === String(row.id));
  if (idx === -1) {
    await sheets.spreadsheets.values.append({
      spreadsheetId: SPREADSHEET_ID,
      range: `${SOW_SHEET_NAME}!A2`,
      valueInputOption: "RAW",
    ...
    await sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range: `${SOW_SHEET_NAME}!A${sheetRow}:L${sheetRow}`,
      valueInputOption: "RAW",
      requestBody: { values: [rowValues] },
    });
  }
...
export async function deleteSOWRow(id) {
  ...
  await sheets.spreadsheets.batchUpdate({
    spreadsheetId: SPREADSHEET_ID,
    requestBody: { requests: [{ deleteDimension: { range: { sheetId: sheetMeta.properties.sheetId, dimension: "ROWS", startIndex: sheetRow - 1, endIndex: sheetRow } } }] },
  });
}
