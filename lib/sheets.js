...
export async function uploadScreenshot(fileBase64, fileName, postId, platformName) {
  ...
  if (!meta.data.sheets.find(s => s.properties.title === SCREENSHOTS_SHEET)) {
    ...
    await sheets.spreadsheets.values.update({
      ...
      valueInputOption: "RAW",
      requestBody: { values: [["Post ID", "Platform", "File Name", "Uploaded At", "Base64 Data", "Status", "Comment"]] }
    });
  }
  ...
  await sheets.spreadsheets.values.append({
    ...
    valueInputOption: "RAW",
    requestBody: { values: [[postId, platformName, fileName || "screenshot.png", now, fileBase64, "confirmed"]] }
  });
...
export async function getScreenshot(postId, platformName) {
  ...
  try {
    ...
    const rows = res.data.values || [];
    // Find latest confirmed screenshot for this post+platform
    const matches = rows.filter(r => String(r[0]) === String(postId) && r[1] === platformName && (r[5] === "confirmed" || !r[5]));
...
// ── SCREENSHOT REPLACEMENT REQUEST (Posting team) ────────────
export async function requestSSReplacement(fileBase64, fileName, postId, platformName) {
  const sheets = getSheets();
  const SPREADSHEET_ID = process.env.SPREADSHEET_ID;
  ...
  // Mark any existing pending rows for this post+platform as superseded
  const existing = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
    range: `${SCREENSHOTS_SHEET}!A2:G5000`,
  });
  const rows = existing.data.values || [];
  const updates = [];
  rows.forEach((row, i) => {
    if (String(row[0]) === String(postId) && row[1] === platformName && row[5] === "pending") {
      updates.push({ range: `${SCREENSHOTS_SHEET}!F${i + 2}`, values: [["superseded"]] });
    }
  });
  if (updates.length) {
  ...
  await sheets.spreadsheets.values.append({
    ...
    range: `${SCREENSHOTS_SHEET}!A2`,
    valueInputOption: "RAW",
    requestBody: { values: [[postId, platformName, fileName || "screenshot.png", now, fileBase64, "pending"]] }
  });

  await logActivity("SS Replacement Requested", postId, `${platformName} replacement requested`, "Posting Team");
  return `ss:${postId}:${platformName}`;
}
...
export async function getPendingApprovals() {
  ...
  const SPREADSHEET_ID = process.env.SPREADSHEET_ID;
  try {
    const [ssRes, postsRes] = await Promise.all([
      sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: `${SCREENSHOTS_SHEET}!A2:F5000` }),
      sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: `${SHEET_NAME}!A2:AD2000` }),
    ]);
    const ssRows = ssRes.data.values || [];
    const postRows = postsRes.data.values || [];

    const pending = ssRows
      .map((row, i) => ({ row, sheetRow: i + 2 }))
      .filter(({ row }) => row[5] === "pending" || row[5] === "rejected");

    return pending.map(({ row, sheetRow }) => {
      const postId = String(row[0]);
      const platformName = row[1];
      const postRow = postRows.find(r => String(r[COL.ID - 1]) === postId);
      // Get confirmed (old) screenshot
      const confirmedRows = ssRows.filter(r => String(r[0]) === postId && r[1] === platformName && (r[5] === "confirmed" || !r[5]));
      const oldSS = confirmedRows.length ? confirmedRows[confirmedRows.length - 1] : null;
      return {
        sheetRow,
        postId,
        platformName,
        ...
        uploadedAt: row[3],
        newBase64: row[4],
        oldBase64: oldSS ? oldSS[4] : null,
        postTitle: postRow ? postRow[COL.TITLE - 1] : "",
        client: postRow ? postRow[COL.CLIENT - 1] : "",
...
// ── APPROVE SCREENSHOT REPLACEMENT ───────────────────────────
export async function approveSSReplacement(postId, platformName) {
  const sheets = getSheets();
  const SPREADSHEET_ID = process.env.SPREADSHEET_ID;
  ...
  rows.forEach((row, i) => {
    if (String(row[0]) === String(postId) && row[1] === platformName) {
      if (row[5] === "pending") {
        // Approve this one
        updates.push({ range: `${SCREENSHOTS_SHEET}!F${i + 2}`, values: [["confirmed"]] });
      } else if (row[5] === "confirmed" || !row[5]) {
        // Supersede old confirmed
        updates.push({ range: `${SCREENSHOTS_SHEET}!F${i + 2}`, values: [["superseded"]] });
      }
    }
  });
  ...
  await logActivity("SS Replacement Approved", postId, `${platformName} replacement approved by PM`, "PM");
  return `ss:${postId}:${platformName}`;
}
...
// ── REJECT SCREENSHOT REPLACEMENT ────────────────────────────
export async function rejectSSReplacement(postId, platformName, comment) {
  const sheets = getSheets();
  const SPREADSHEET_ID = process.env.SPREADSHEET_ID;
  ...
  rows.forEach((row, i) => {
    if (String(row[0]) === String(postId) && row[1] === platformName && row[5] === "pending") {
      updates.push({ range: `${SCREENSHOTS_SHEET}!F${i + 2}:G${i + 2}`, values: [["rejected", comment || ""]] });
    }
  });
  ...
  await logActivity("SS Replacement Rejected", postId, `${platformName} replacement rejected by PM${comment ? ": " + comment : ""}`, "PM");
  return true;
}
