...
export async function uploadScreenshot(fileBase64, fileName) {
  // Upload to imgbb (free anonymous image hosting) — returns a direct URL
  // we can store in the sheet and render with =IMAGE()
  const formData = new URLSearchParams();
  formData.append("image", fileBase64);
  formData.append("name", fileName || "screenshot");
  ...
  if (!data.success) throw new Error("Image upload failed: " + JSON.stringify(data.error));
